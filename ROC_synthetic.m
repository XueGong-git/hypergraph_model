...
gamma_inp = 10; % decay parameter for synthetic networks
c2_inp = 1;
c3_inp = 0.3;
m = 60; % number of nodes per cluster

c2 = 1; % weight of diadic edge
c3_array = 0:0.1:1; % weight of triadic edge
...
for trial_id = 1:ntrial
    ...
    while isConnected == 0
        ...
        g = digraph(W2+W3); bins = conncomp(g); isConnected = all(bins == 1); %check if train graph is connected
    ...
    %%%%%%%%% arithmetic mean %%%%%%%%%%%
    [X_arith,Y_arith,T_arith,AUC_triangle_arith(trial_id)] = perfcurve(test.T3(trg_open_idx),score_arith(trg_open_idx),1,'XCrit','reca','YCrit','prec','TVals', linspace(max(score_arith(trg_open_idx), [], 'all'),1/3));

    %%%%%%%%%% harmonic mean %%%%%%%%%%%%
    [X_harm,Y_harm,T_harm,AUC_triangle_harm(trial_id)] = perfcurve(test.T3(trg_open_idx),score_harm(trg_open_idx),1,'XCrit','reca','YCrit','prec','TVals', linspace(max(score_harm(trg_open_idx), [], 'all'),1));

    %%%%%%%% geometric mean %%%%%%%%%
    [X_geo,Y_geo,T_geo, AUC_triangle_geo(trial_id)] = perfcurve(test.T3(trg_open_idx),score_geom(trg_open_idx),1,'XCrit','reca','YCrit','prec','TVals', linspace(max(score_geom(trg_open_idx), [], 'all'),1));

    %%%%%% random score %%%%%%%%%%
    [X_rand,Y_rand,T_rand, AUC_triangle_rand(trial_id)] = perfcurve(test.T3(trg_open_idx), rand(size(trg_open_idx)),1,'XCrit','reca','YCrit','prec');
    ...
    for ii = 1:length(c3_array)
      ...
      %  [~, eta_periodic_scaled, ~] = CalculateModelLikelihood(x_est_linear, W2_in, T3_in, c2, c3, 2, "linear");
        ...
        %triangle
        [X_lin,Y_lin,T_lin,AUC_triangle_lin(trial_id, ii)] = perfcurve(test.T3(trg_open_idx),P_triangles_lin(trg_open_idx),1,'XCrit','reca','YCrit','prec');
        %plot(X_lin,Y_lin)
        %xlabel('Recall') 
        %ylabel('Precision')
...
fprintf('Training Ratio: %.2g\n', train_ratio)
fprintf('Rand: %.2g\n', mean(AUC_triangle_rand))
fprintf('Linear: %.2g\n', max(mean(AUC_triangle_lin)))
fprintf('Arith: %.2g\n', mean(AUC_triangle_arith))
fprintf('Geo: %.2g\n', mean(AUC_triangle_geo))
fprintf('Harm: %.2g\n', mean(AUC_triangle_harm))
